...
export function generateDeliberationPDF(data: ExportData): void {
  ...
  const getNote = (etudiantId: string, ecueId: string): number | null => {
    return (
      notes.find((n) => n.etudiant_id === etudiantId && n.ecue_id === ecueId)
        ?.note ?? null
    );
  };
  ...
  const logoSize = 18;
  try {
    doc.addImage(logoPath, "PNG", pageWidth / 2 - logoSize / 2, 3, logoSize, logoSize);
  } catch (error) {
  ...
  doc.text("GRILLE DE DELIBERATION", margin, 20);

  doc.setFontSize(11);
  doc.text(`ANNEE ACADEMIQUE: ${session.annee_academique}`, margin, 28);

  // Centré (sous le logo)
  doc.setFontSize(11);
  doc.setFont("helvetica", "bold");
  doc.text("REPUBLIQUE DU BURUNDI", pageWidth / 2, 24, { align: "center" });

  doc.setFontSize(10);
  doc.setFont("helvetica", "normal");
  doc.text("MINISTERE DE L'EDUCATION NATIONALE", pageWidth / 2, 29, {
    align: "center",
  });
  doc.text("ET DE LA RECHERCHE SCIENTIFIQUE", pageWidth / 2, 34, { align: "center" });

  doc.setFont("helvetica", "bold");
  doc.text("UNIVERSITE DE NGOZI", pageWidth / 2, 41, { align: "center" });

  // À droite
  doc.setFontSize(10);
  doc.setFont("helvetica", "normal");
  doc.text(`Filière: ${session.filiere}`, pageWidth - margin, 24, {
    align: "right",
  });
  doc.text(`Niveau: ${session.niveau}`, pageWidth - margin, 29, {
    align: "right",
  });
  doc.text(`Semestre: ${session.semestre}`, pageWidth - margin, 34, {
    align: "right",
  });
  if (session.date_deliberation) {
    doc.text(
      `Date: ${new Date(session.date_deliberation).toLocaleDateString(
        "fr-FR"
      )}`,
      pageWidth - margin,
      39,
  ...
  etudiants.forEach((etudiant, index) => {
    const row: any[] = [
      ...
      {
        content: `${etudiant.nom} ${etudiant.prenom}`,
        styles: { halign: "left" },
      },
    ];
    ...
    ues.forEach((ue, ueIndex) => {
      ...
      row.push({
        content:
          resultatUE.moyenne !== null ? resultatUE.moyenne.toFixed(2) : "-",
        styles: {
          halign: "center",
          fontStyle: "bold",
          fillColor:
            resultatUE.decision === "UV"
              ? [220, 240, 220]
              : resultatUE.decision === "UNV"
              ? [255, 220, 220]
              : undefined,
        },
      });
    ...
    row.push({
      content:
        resultatSemestre.moyenne !== null
          ? resultatSemestre.moyenne.toFixed(2)
          : "-",
      styles: {
        halign: "center",
        fontStyle: "bold",
        fillColor:
          resultatSemestre.moyenne !== null && resultatSemestre.moyenne >= 10
            ? [200, 240, 200]
            : [255, 220, 220],
      },
    });
  ...
  autoTable(doc, {
    ...
    columnStyles: {
      0: { cellWidth: 8 }, // N°
      1: { cellWidth: 50 }, // Nom et Prénom (plus large sans matricule)
    },
  ...
  const fileName = `Deliberation_${session.filiere}_${
    session.niveau
  }_${session.annee_academique.replace("/", "-")}_${session.semestre}.pdf`;
  doc.save(fileName);
}
